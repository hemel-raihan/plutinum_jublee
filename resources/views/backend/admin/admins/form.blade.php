...
@section('content')
						...
						<div class="page-header">
							<div>
								<h1 class="page-title">{{ isset($admin) ? 'Edit ' : 'Create '}}Users</h1>
								{{-- <ol class="breadcrumb">
							...
							<div class="ms-auto pageheader-btn">
								<a href="{{route('admin.admins.index')}}" class="btn btn-primary btn-icon text-white me-2">
									<span>
								...
								<div class="card">
									...
									<div class="card-body">
                                            ...
                                            <div class="form-group">
													...
													<input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="exampleInputname" value="{{$admin->name ?? old('name')}}" placeholder="Enter user name">
                                                    @error('name')
												...
												<div class="form-group">
													...
													<input type="email" class="form-control @error('email') is-invalid @enderror" name="email" id="exampleInputEmail1" value="{{$admin->email ?? old('email')}}" placeholder="Enter email">
                                                    @error('email')
                                                           <span class="invalid-feedback" user="alert">
                                                               <strong>{{ $message }}</strong>
                                                           </span>
                                        ...
                                        <label for="role">Select Role</label>
                                            <select id="role"  class="js-example-basic-single form-control @error('role') is-invalid @enderror" name="role_id">
                                                @foreach ($roles as $role )
                                                    <option value="{{ $role->id }}" @isset($user) {{$admin->role->id == $role->id ? 'selected' : ''}} @endisset>{{$role->name}}</option>
                                                @endforeach
                                            </select>
