...
@section('content')
	...
	<div class="row">
        ...
        <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom  w-100">
            ...
            <tbody>
            @foreach($page->galleryimges as $asd)
            @php
            $imagee = explode("|",$asd->image);
            @endphp
            @foreach ($imagee as $key => $test)

    <tr>
        <td>
            <img src="{{asset('uploads/pagegallary_image/'.$test)}}" width="50"></td>

        <td>

            <a href="{{route('admin.galleryimg.deletee',['id'=>$asd->id,'key'=>$key])}}" class="btn btn-success">
            <i class="fa fa-trash"></i>
            </a>


         <button class="btn btn-danger waves effect" type="button" onclick="deletepost$page({{ $asd->id}})" >
                <i class="fa fa-trash"></i>
            </button>
            <form id="deleteform-{{$asd->id}}" action="#" method="POST" style="display: none;">
                @csrf
                @method('DELETE')
            </form>
            </td>
		...
		<div class="col-lg-3 col-xl-3 col-md-12 col-sm-12" style="float: left">
			...
			<div class="card">
				...
				<div class="card-body">
                    ...
                    {{-- <form method="POST" action="{{isset($page) ? route('admin.galleryimg.update',$page->id) : route('admin.pages.store')}}" enctype="multipart/form-data">
                        @csrf
                        @isset($page)
                        @method('PUT')
                        @endisset
                        <div class="form-group">
                            ...
                            <!-- <input id="demo" type="file" name="image" accept=".jpg, .png, image/jpeg, image/png" multiple="" class="ff_fileupload_hidden"> -->
                            <input type="file" class="dropify @error('gallaryimage') is-invalid @enderror form-control" data-default-file="{{ isset($page) ? asset('uploads/pagegallary_image/'.$page->gallaryimage) : '' }}" multiple name="gallaryimage[]">
                            @error('gallaryimage')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{$message}}</strong>
                            </span>
                    ...
                    <div class="form-group">
                        ...
                        {{-- <div class="form-group">
                            <label class="form-label">Gallary Image</label>
                            <input id="demo" type="file" name="gallaryimage" accept=".jpg, .png, image/jpeg, image/png" multiple style="display: none;">
                        </div> --}}
