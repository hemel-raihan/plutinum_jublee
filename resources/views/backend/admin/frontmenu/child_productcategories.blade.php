...
<ul class="transfer-double-group-list-li-ul transfer-double-group-list-li-ul-1636878492751">
    @foreach($sub_category->childrenRecursive as $key => $sub)
        ...
        <div class="checkbox-group">
                <input type="checkbox" name="productcategorybox[]" value="{{$sub->name}}" data-src="{{$sub->id}}" data-id="{{$sub->slug}}"  class="checkbox-normal group-checkbox-item-1636878492751 belongs-group-0-1636878492751" id="group_<?php echo $sub_category->id; ?>_checkbox_<?php echo $key ?>_1636878492752" />
            ...
            @include('backend.admin.frontmenu.child_productcategories', ['sub_category' => $sub])
            @endif
         </li>
         @endforeach
    </ul>
