...
@section('content')
						...
						<div class="page-header">
							<div>
								<h1 class="page-title">{{ isset($galleryphoto) ? 'Edit ' : 'Create '}}Image</h1>
								{{-- <ol class="breadcrumb">
							...
							<div class="ms-auto pageheader-btn">
								<a href="{{route('admin.galleryphotos.index')}}" class="btn btn-primary btn-icon text-white me-2">
									<span>
                   ...
                   <!-- ROW-1 OPEN -->
    <form method="POST" action="{{isset($galleryphoto) ? route('admin.galleryphotos.update',$galleryphoto->id) : route('admin.galleryphotos.store')}}" enctype="multipart/form-data">
    @csrf
    @isset($galleryphoto)
    @method('PUT')
    @endisset
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="form-group">
						<label for="exampleInputname">Image Title</label>
						<input type="text" class="form-control" value="{{$galleryphoto->name ?? old('name')}}" name="name" id="" placeholder="Image Name">
					</div>
					...
					<div class="form-group">
						...
						<div class="ql-wrapper ql-wrapper-demo bg-light">
                            ...
                            <textarea name="desc" class="my-editor form-control" id="ckeditor" style="height: 200px;" cols="30" rows="10">{!!$galleryphoto->desc ?? old('desc')!!}</textarea>
				...
				<div class="card-footer text-end">
					...
					<a href="{{route('admin.galleryphotos.index')}}" class="btn btn-danger mt-1">Cancel</a>
				</div>
			</div>
		</div>
		...
		<div class="col-lg-3 col-xl-3 col-md-12 col-sm-12" style="float: left">
                                            ...
                                            @foreach($categories as $key => $categoryy)
												...
												<li class="transfer-double-group-list-li transfer-double-group-list-li-1636878492751">
													<div class="checkbox-group">
														...
														<input type="checkbox" name="categories" value="{{$categoryy->id}}" {{$categoryy->id == $galleryphoto->gallerycategory_id ? 'checked' : ''}} class="checkbox-normal group-select-all-1636878492751" id="group_{{$key}}_1636878492751" /><label for="group_{{$key}}_1636878492751" class="group-name-1636878492751">{{$categoryy->name}}</label>
			...
			<div class="card">
				...
				<div class="card-body">
                    ...
                    <div class="form-group">
						<label class="form-label">Feature Image</label>
						<input type="file" name="image" class="dropify @error('image') is-invalid @enderror" data-default-file="{{ isset($galleryphoto) ? asset('uploads/gallery_photo/'.$galleryphoto->image) : ''}}" data-bs-height="180"  />
                        @error('image')
