...
@section('content')
						...
						<div class="page-header">
							<div>
								<h1 class="page-title">{{ isset($teampost) ? 'Edit ' : 'Create '}}Posts</h1>
								{{-- <ol class="breadcrumb">
                   ...
                   <!-- ROW-1 OPEN -->
    <form method="POST" action="{{isset($teampost) ? route('admin.teamposts.update',$teampost->id) : route('admin.teamposts.store')}}" enctype="multipart/form-data">
    @csrf
    @isset($teampost)
    @method('PUT')
    @endisset
		...
		<div class="col-lg-9 col-xl-9 col-md-12 col-sm-12">
			<div class="card">
				...
				<div class="card-footer text-end">
					...
					<a href="{{route('admin.teamposts.index')}}" class="btn btn-danger mt-1">Cancel</a>
				</div>
			</div>
		</div>
                                            ...
                                            @foreach($categories as $key => $categoryy)
												...
												<li class="transfer-double-group-list-li transfer-double-group-list-li-1636878492751">
													<div class="checkbox-group">
														...
														<input type="checkbox" name="categories[]" value="{{$categoryy->id}}" @foreach($teampost->teamcategories as $category) {{$categoryy->id == $category->id ? 'checked' : ''}} @endforeach class="checkbox-normal group-select-all-1636878492751" id="group_{{$key}}_1636878492751" /><label for="group_{{$key}}_1636878492751" class="group-name-1636878492751">{{$categoryy->name}}</label>

													</div>
                                                    @if($categoryy->childrenRecursive->count()>0)


													  @include('backend.admin.team.post.child_category_edit', ['sub_category' => $categoryy,'post' => $teampost])
                                            ...
                                            @if($category->status == true)
												...
												<li class="transfer-double-group-list-li transfer-double-group-list-li-1636878492751">
                                                    ...
                                                    @if($category->childrenRecursive->count()>0)
													  ...
													  @include('backend.admin.team.post.child_categories', ['sub_category' => $category])
				...
				<div class="card-body">
					...
					<div class="form-group">
						...
						<label class="custom-switch">
							<input type="checkbox" name="status" {{$teampost->status == true ? 'checked' : ''}} class="custom-switch-input ">
